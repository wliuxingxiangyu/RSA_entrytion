			...
			if (bundle != null) {
				...
				if (messages.length > -1) {
					...
					Notification notifyDetails = new Notification(
							R.drawable.ic_launcher, "message received",
							System.currentTimeMillis());
					// PendingIntent myIntent =
					// PendingIntent.getActivity(context, 0, new
					// Intent(Intent.ACTION_VIEW), 0);
					PendingIntent myIntent = PendingIntent.getActivity(context,
							0, defineIntent, 0);

					notifyDetails
							.setLatestEventInfo(context, str, "", myIntent);
					notifyDetails.flags = Notification.FLAG_AUTO_CANCEL;
					notifyDetails.flags = Notification.DEFAULT_SOUND;
					mNotificationManager.notify(SIMPLE_NOTFICATION_ID,
							notifyDetails);
					Toast.makeText(context, "el rsala " + str,
